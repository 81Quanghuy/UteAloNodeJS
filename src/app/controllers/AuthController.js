...
class AuthoController {
	...
	async sendOTP(req, res, next) {
		try {
			...
			// save otp to redis set time expire 5m
			const userID = await User.findOne({ userName: req.body.fullName }).select('_id');
			await redisClient.set(`comfirm:${userID}`, otp, 'EX', 60 * 5);

			// Gửi mã OTP đến email
			const status = await sendMailOTP(req.body.email, 'OTP Comfirm Set Password', otp, req.body.fullName);
			// check status
			if (!status) {
				// delete in redis
	...
	async register(req, res, next) {
		try {
			...
			// create new user
			const roleId = await Role.findOne({ roleName: req.body.roleName }).select('_id');
			...
			if (roleId.roleName === 'SinhVien') {
				const postGroup = await PostGroup.findOne({ postGroupName: req.body.groupName });
	...
	async login(req, res, next) {
		try {
			...
			return res.status(200).json({
				success: true,
				message: 'Sign Up Success',
				result: { accessToken, refreshToken, userId: user._id },
				statusCode: 200,
			});
		} catch (err) {
			console.log(err);
